...
| 28 | **`DerivativeParsedMaterial` 只对「表达式里字面出现的变量」发射导数 —— 经 `material_property_names` 引来的依赖不参与发射** | T1b 卡了好几轮（"两种修复都动不了 FD 比值"）。本轮用 `[Debug] show_material_props = true` 拿到**权威清单**：`L2a`/`free_energy` 全产出；而 `L2b`（表达式只有 `align4`）、`L_aniso`（表达式只有 `L2a L2b`）、`solute_mobility`（表达式只有 `S_eta2 h_gb h_solid`）**只产出基属性，导数一个都没有**。<br>受控 FD 判决（`run_t1b_L_test.sh`，4 档只差两处）：正对照 `6.17e-10` ✅ / 生产结构 `1.36e-03` ❌ / **把 η 依赖写进 `L2b` 的表达式仍 `1.36e-03`** ❌ / 绕过 `L_aniso` `1.23e-09` ✅。<br>⚠ **当年那条"给 `L2b` 加 `coupled_variables` 就够了"的修复实测无效** —— 而且看比值逐位相同就知道：断链在**上一层**。<br>⇒ **给材料加 `coupled_variables` 并不能让 MOOSE 发射导数**；要么把依赖**字面写进表达式**，要么别用中间层。**判据是 `[Debug] show_material_props` 的产出清单 + 小算例 FD 双判据，不是"我加了那一行"** |
| 29 | **`grep -c` 数不出来时先怀疑正则，别急着下结论** | 抓 MOOSE 的 `[Debug]` dump 时我按 `(N) name : type` 写正则，抓到 0 个属性，差点得出"属性一个都没有"。实际 MOOSE 打印的是 **`Active Materials:` + `Property Names:`** 两段（产出侧）和 `Consumed Material Properties:`（消费侧），格式完全不同。⇒ **先 `head` 看原始文本再写正则** |

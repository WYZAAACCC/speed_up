...
def main():
    ...
    # --- 3a.【AD 转换】熔化开关 barrier_mu 必须也是 AD 材料 ---
    #
    # `ADGrainGrowth` 用 `getADMaterialProperty<Real>("mu")` 取 mu。MOOSE 禁止同一
    # 属性既作 AD 又作非 AD 声明，所以 barrier_mu 必须从 DerivativeParsedMaterial
    # 换成 ADDerivativeParsedMaterial，否则报
    #   "The requested AD material property 'mu' ... is already ... non-AD"
    # 已核对：mu 在生产输入里没有非 AD 消费者（溶质系统用的是 F/w/M 等别的属性）。
    mb = sub_block_span(out, "barrier_mu")
    block = out[mb[0]:mb[1]]
    if "type = DerivativeParsedMaterial" not in block:
        sys.exit("错误：[Materials]/barrier_mu 不是 DerivativeParsedMaterial，拒绝继续")
    out = (out[:mb[0]]
           + block.replace("type = DerivativeParsedMaterial",

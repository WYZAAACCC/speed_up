...
prob = """  # =====================================================================
  ...
  #
  # ⚠⚠ **参数来源必须分清**（论文里照这个写）。
  #   **2026-09-20 更新：三个指派值里有两个换成了理论/文献值** ——
  #   完整账本见 `validated/PARAMETER_PROVENANCE.md`。
  #   | 参数 | 值 | 来源 | 档 |
  #   |---|---|---|---|
  #   | Tm   | 1923 K      | 文献（Ti64 液相线） | L |
  #   | ΔH_f | 1.28e9 J/m³ | 文献（2.9e5 J/kg × 4430 kg/m³） | L |
  #   | γ    | **0.198** J/m² | **MD 直接算的**：Kavousi et al., MSMSE 28 (2020) 015006<br>Table 12 纯 Ti `γ0 = 198(3) mJ/m²`（2NN-MEAM + 毛细涨落法） | **L** |
  #   | k    | 0.6303      | 模型自洽（由 A_part 定，T4 验证过） | 自洽 |
  #   | mL   | **819** K/摩尔分数 | **van't Hoff 理论**：`−R·T_m²(1−k)/ΔH_f` | **T** |
  #   | f(θ) | 0.00267     | ⚠ 仍是指派值（θ=20°）—— 但**可以用 free-growth 整个换掉**，见下 | A |
  #   | A    | 1e6 /s      | ⚠ 仍是指派值 —— 物理对应物是「颗粒数密度 × 27% 形核效率」 | A |
  #
  # 【下一步：把 f(θ) 换掉（文献支持的路线）】
  #   Metall. Mater. Trans. A 51 (2020) 4360（DOI 10.1007/s11661-020-05880-4）给出
  #   两个**不需要接触角**的替代量：
  #     * 生长限制因子  Q = m_L·C0·(k−1) = (−819)×0.036×(0.6303−1) = **10.90 K**（纯理论）
  #     * Greer free-growth  ΔT_n = 4·γ_SL/(ΔS_f,v·d_p)   ← 只需要**颗粒直径 d_p**
  #   后者比接触角好找得多（SEM/TEM 可测，LPBF 夹杂物尺寸有大量文献）。
  #   ⚠ 该综述同时**独立确认了本处实现的 CET 机制**（ΔT_CS > ΔT_n ⇒ 等轴晶）。
  #
  ...
  [nuc_probability]
    ...
    # 【2026-09-20 参数溯源更新】见 validated/PARAMETER_PROVENANCE.md §1
    #   gamma : 0.2      → **0.198**  （**L 档：MD 直接算出来的**）
    #         Kavousi et al., MSMSE 28 (2020) 015006, DOI 10.1088/1361-651X/ab580c
    #         Table 12：纯 Ti 的 γ0 = **198(3) mJ/m²**（2NN-MEAM，毛细涨落法）
    #         ⚠ 本轮我先按 JOM 2018 的 Γ=1.88e-7 K·m 反推出 0.124，**那个是错的** ——
    #           Γ 与 ΔH_f 的口径不同（Γ 隐含的 ΔS_f,v 比 1.28e9/1941 大 1.6 倍）。
    #           MD 的 198 直接证实了原来那个 0.2 是对的。**教训：反推值要拿独立来源校。**
    #   mL    : 600      → **819**    （T 档：van't Hoff  m_L = −R·T_m²(1−k)/ΔH_f
    #                                  = −8.314×1941²×0.3697/14150 = −819 K/摩尔分数）
    #                                  ⚠ JOM 2018 给的 −0.088 K/% 与 Ti64 相图不符（差 30 倍以上），
    #                                    不能用；见 §1.2
    constant_expressions = '1923 1.28e9 0.198 1.380649e-23 0.002673 1.0e6 0.036 0.6303 819'

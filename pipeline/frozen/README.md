...
### 5.6 【2026-09-20】**✅ 已合入生产**（见 §5.9）：抗截留项（缺口 #3 的修复主体）

> ⚠ 本节写于合入之前。**2026-09-20 用户决定「做」，已落进 `stage1_meltpool_c.i`** ——
> 现在的状态以 §5.9 为准，本节保留为设计与验收记录。
...
| **不拖慢牛顿收敛** | ✅ 1D 757 个时间步的 **NL 迭代总数**：基线 `2268` → `ALPHA=2` **`2274`（+0.26%）**；`ALPHA=3` `2285`（+0.75%）；`s=2` 上 `2268 → 2277`（+0.40%）。<br>（与拖曳项验收同一把尺子：**"看不出代价"才算通过**）|
...
### 5.8 【2026-09-20】**✅ 已合入生产**（见 §5.9）：`D_L` 子网格闭合（缺口 #3 第 3 条）

> ⚠ 本节写于合入之前。**2026-09-20 用户决定「做」，已落进 `stage1_meltpool_c.i`** ——
> 现在的状态以 §5.9 为准，本节保留为推导与副作用核算记录。

**为什么需要**：生产工作点上 `δ_c = D_L/V = 4.2 nm`，比 `dx = 1 µm` 小 **238 倍**
⇒ 网格不可解析 ⇒ 那层被摊到 `~dx` 宽。实测（`run_prod_1d.sh`）：
`k_eff = 0.999015` vs 物理 `0.6549` ⇒ **界面排出的溶质被低估 350 倍**。

**改动**：`[solute_mobility]` 的 `D_L: 2.52e-9 → 1.2e-6`（`D_S`/`D_GB`/`k_c`/`A_part` 不动）。

取值理由：要求 `δ_c ≥ 2·dx` ⇒ `D_L ≥ 2·V_scan·dx = 2×0.6×1e-6 = 1.2e-6`。
此时 `δ_c = 2 µm`、`ξ/δ_c = 1` —— **正是抗截留项（5.6）残差最小（−0.69%）的那一档**。

**为什么不引入新近似**：精确关系 `c_max − c0 = 2·M·A·c0/(V·L_eff)`，
抗截留项把 `L_eff` 拉回 `δ_c = M·k_c/V` 后 ⇒ `c_max − c0 = 2A·c0/k_c`，
**与 `D_L`、与网格都无关**。所以放大只是把"物理上本就有、但网格看不见"的那层
变成可解析的。

**副作用逐条核算（`run_dl_check.sh` 实测）**：

| 关注点 | 结果 |
|---|---|
| 熔池尺度混合 | `D_L·t/L² = 1.2e-6 × 1.7e-4/1e-8 = 0.02 ≪ 1` ⇒ 仍不混合 ✅ |
| **`D_S` / `D_GB`** | T6 实测**逐点精确不变**（固相 `4.0e-13`、晶界 `4.0e-10`）⇒ **不影响 `s·δ·D_GB`** ✅ |
| 平衡分配（T4） | `V = 0`，与 `D` 无关 ⇒ 不受影响 ✅ |
| 矩阵条件数 | `M/dx²` 更接近 `1/dt` ⇒ 反而改善 ✅ |
| **时间步** | ⚠ **自适应步长被压小**：实测 86×30 上 `dt` 从 `6.33e-7` 掉到 `3.38e-7`，**步数翻倍 ⇒ 约 2× 墙钟** |

⚠ **这是本修复唯一的代价，必须记账。** 生产 `dx = 1 µm` 上第 4 阶稳定限
`dx⁴/(16·M·κ_c) = 4.7e-6` 仍 > `dtmax = 2e-6`；但 **AMR 细到 `dx = 0.5 µm` 时
掉到 `2.9e-7`**，那时代价可能到 ~7×。⇒ **合入前要用生产网格实测总账。**

⚠ **必须如实声明**：`1.2e-6` **不是** Ti64 中 V 的液相扩散系数（那是 `2.52e-9`），
而是**子网格闭合参数**。论文里必须写成 closure，不能写成材料常数。

---

### 5.9 【2026-09-20】**已合入生产**：缺口 #3 的 (b)+(c)（抗截留 + `D_L` 闭合）

**用户 2026-09-20 决定「做」。** 两处改动都直接落在 `stage1_meltpool_c.i` 上
（改前另存 `stage1_meltpool_c.preantitrap_dl.i`），因此 **5.6 / 5.8 两节的
「候选」措辞作废，它们现在是生产默认。**

```
stage1_meltpool_c.i   775f9487734989406bac33c57ff7760f3e2442d0d9b81e8f0c7a5cb6f4306669   1107 行
                      （改前：76ecbe12226eaaf04a2f6e4d4443c93e74fff03d8190d32e8cdb9b02facdd838，1003 行）
```

| # | 改动 | 生成器 |
|---|---|---|
| (b) | 8 个 `AntitrappingCurrent` + `[at_susc]`；`F_at = 2·2e-6·(1−0.63)·c·(1−h_gb) + 0*w` | `../validated/make_antitrap_prod.py` |
| (c) | `[solute_mobility]` 的 `D_L: 2.52e-9 → 1.2e-6`（**子网格闭合**，`D_S`/`D_GB` 不动） | `../validated/make_dl_prod.py` |

**验收（`../validated/run_antitrap_check.sh`，走完整条链）**：
链 = `gen → splice → jacfix → jacchain`，**22 项存活检查全过** +
`--check-input` → **`Syntax OK`**（setup 348 s）。

⚠ 那次验收里有三条断言原本查的是 **jacchain 之前**的字面拼法
（`(1-h_gb)`、`material_property_names = 'h_gb'`、`min(1, 2*S_eta2)`）。
jacchain 把这些内联之后**那些字面形式本来就该消失**，继续那样查会把
"修好了"报成"丢了"。已改成查语义（表达式里必须有内联后的结构）。

⚠ **代价**：抗截留项几乎免费（NL 迭代 +0.26%）；`D_L` 闭合约 **2×** 墙钟
（AMR 细到 0.5 µm 时可能 ~7×）。见 5.8。
